...
class HandlerSmokePhone(HandlerBase):
    def __init__(self,cp,modelPath="",gpuID:str="0",gpuNums=1):
        super(HandlerSmokePhone, self).__init__()
        self.cp = cp
    ...
    def handle(self,message:ZmqMessage):
        ...
        return self.smokeClass(message.st,message.timePoint)
    ...
    def preDetect(self,modelPath,gpuId:str,gpuNums):
        ...
        :return:
        """
        path = os.path.join(modelPath,"smoke_phone_class")
        self.smokeModelPath = os.path.join(path,"smokeAndPhone.pth")
        # print("smokeModelPath Path is {}".format(self.helmetModelPath))

        #TODO:debug cpu model
        # self.device = select_device('cpu')

        self.metaPath_smoke = os.path.join(path,'smoke_classIndices.json')
        self.metaPath_phone = os.path.join(path,'phone_classIndices.json')

        tprint("SMOKE&PHONE model device init ")

        self.device = "cuda:{}".format(gpuId) if gpuId != "cpu" else "cpu"
        tprint("SMOKE&PHONE model load start")
        self.model = getModel(2,self.smokeModelPath,self.device)
        tprint("SMOKE&PHONE model load end")


    def smokeClass(self,request,timePoints):
        """
        ...
        # XXX
        ret = getPredict(img, self.metaPath_smoke,self.metaPath_phone, self.model, device=self.device)

        torch.cuda.synchronize()
        end  = time.time()
        tprint("smoke detect spend time {}".format(round((end-start)*1000,4)))
